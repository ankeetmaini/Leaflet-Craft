...
test.beforeEach(t => {
  ...
  t.context.freeDraw = new FreeDraw({
    mergePolygons: false,
...
test('It should be able to trigger events on the map instance;', t => {
  ...
  const closedPolygon = [
    ...firstPolygon.getLatLngs()[0],
    firstPolygon.getLatLngs()[0][0],
  ];
  t.truthy(
    freeDraw.fire.calledWith('markers', {
      latLngs: [closedPolygon],
      eventType: 'create',
    }),
  );
});
